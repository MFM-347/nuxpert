    ...
    <div class="relative flex flex-col items-center gap-8 text-center">
      ...
      <div class="flex items-center gap-3">
        ...
        <span class="text-[42px] font-bold tracking-tight">
          Nuxt<span class="ml-2" :class="isPro ? 'text-violet-500' : 'text-green-500'"
            >SEO{{ isPro ? ' Pro' : '' }}</span
          >
        </span>
      </div>
      ...
      <!-- Title -->
      <h1 class="m-0 max-w-[1000px] text-[80px] leading-tight font-bold">
        {{ title }}
      </h1>

      <!-- Description -->
      <p v-if="description" class="max-w-[900px] text-[32px] leading-relaxed opacity-70">
        {{ description }}
      </p>
    </div>
  </div>
